...
<html>
    <head>
        ...
        <!-- Ajouter le CDN FontAwesome pour l'icône d'humidité -->
            ...
            $(function () {
                $('#container').highcharts({
                    ...
                    {   name: 'Humidité',
                        data: [
                        <?php
                            $query = " select humidity from table_capteurs order by id desc limit 100 ";
                            $resultados = mysqli_query($connection, $query);
                            while($rows = mysqli_fetch_array($resultados)){
                                ?>
                                    <?php echo $rows["humidity"]?>,
                                <?php
                            }
                        ?>
                        ]
